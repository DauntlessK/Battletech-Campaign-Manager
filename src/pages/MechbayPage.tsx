...
export default function MechbayPage({ campaign, force, onBack, onResourceBalanceChange }: { campaign: Campaign; force?: Force; onBack: () => void; onResourceBalanceChange?: (resourceType: "Warchest" | "CBills", balance: number) => void }) {
  ...
  const knownRepairCost = useMemo(() => repairRows.reduce((sum, row) => sum + (row.replacementCostCBills == null ? 0 : Number(row.replacementCostCBills)), 0), [repairRows]);
  const unavailableRepairCosts = useMemo(() => repairRows.filter((row) => row.replacementCostCBills == null).length, [repairRows]);
  ...
  return <section className="space-y-5">
    ...
    <div className="flex flex-col gap-3 rounded-2xl border border-zinc-800 bg-zinc-950/70 p-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      {!isChaos && <div className="flex flex-col gap-2 sm:flex-row sm:items-center"><div className="rounded-xl border border-lime-400/25 bg-lime-400/10 px-4 py-2 text-right"><div className="text-[10px] font-black uppercase tracking-[0.16em] text-lime-300">Known Repair Cost</div><div className="text-sm font-black text-lime-100">{knownRepairCost.toLocaleString()} C-bills</div>{unavailableRepairCosts > 0 && <div className="text-[10px] font-semibold text-amber-300">{unavailableRepairCosts} cost{unavailableRepairCosts === 1 ? "" : "s"} unavailable</div>}</div><button type="button" onClick={() => setPartsOpen(true)} className="inline-flex items-center justify-center gap-2 rounded-xl border border-cyan-400/35 bg-cyan-400/10 px-4 py-2 text-xs font-black text-cyan-200"><PackageSearch size={16}/> Parts Requisitions</button></div>}
    </div>
...
function RepairQueue({ rows }: { rows: RepairRow[] }) {
  const knownRows = rows.filter((row) => row.replacementCostCBills != null);
  const unresolvedCount = rows.length - knownRows.length;
  const total = knownRows.reduce((sum, row) => sum + Number(row.replacementCostCBills), 0);
  return <div className="rounded-3xl border border-zinc-800 bg-zinc-950/60"><div className="flex items-center justify-between border-b border-zinc-800 px-5 py-4"><div><h2 className="font-black text-zinc-100">Repair Queue</h2><p className="text-xs text-zinc-500">Repair, replacement, and rearm work generated from official battle damage.</p></div><div className="text-right"><div className="text-xs uppercase text-zinc-500">Known potential cost</div><div className="font-black text-lime-200">{total.toLocaleString()} C-bills</div>{unresolvedCount > 0 && <div className="mt-1 text-xs font-semibold text-amber-300">{unresolvedCount} cost{unresolvedCount === 1 ? "" : "s"} unavailable</div>}</div></div><div className="max-h-[34rem] overflow-auto"><table className="w-full min-w-[1080px] text-left text-sm"><thead className="sticky top-0 z-10 bg-zinc-900 text-xs uppercase tracking-wider text-zinc-500"><tr><th className="px-4 py-3">Unit</th><th className="px-4 py-3">Location</th><th className="px-4 py-3">Work</th><th className="px-4 py-3">Action</th><th className="px-4 py-3 text-center">2D6</th><th className="px-4 py-3">Tech</th><th className="px-4 py-3">Availability</th><th className="px-4 py-3 text-right">Cost</th><th className="px-4 py-3">Status</th></tr></thead><tbody className="divide-y divide-zinc-800">{rows.length ? rows.map((row) => <tr key={row.id}><td className="px-4 py-3 font-black text-zinc-100">{row.unitName}</td><td className="px-4 py-3 text-zinc-400">{prettyLocation(row.locationId)}</td><td className="px-4 py-3 text-zinc-200">{row.quantity > 1 ? `${row.quantity} × ` : ""}{row.itemName}</td><td className="px-4 py-3"><Tag text={row.action} className={actionClass(row.action)}/></td><td className="px-4 py-3 text-center font-black text-zinc-200">{row.repairRoll ?? "—"}</td><td className="px-4 py-3 text-zinc-300">{row.techRating ?? "—"}</td><td className="px-4 py-3 text-zinc-300">{row.availabilityRating ?? "—"}</td><td title={row.costUnavailableReason} className={`px-4 py-3 text-right font-black ${row.replacementCostCBills == null ? "text-amber-300" : "text-zinc-200"}`}>{row.replacementCostCBills == null ? "—" : Number(row.replacementCostCBills).toLocaleString()}</td><td className="px-4 py-3 text-zinc-400">{row.status}</td></tr>) : <tr><td colSpan={9} className="px-4 py-10 text-center text-zinc-500">No repair work is currently queued.</td></tr>}</tbody></table></div></div>;
}
